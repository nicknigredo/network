      ...
      <div style={{ overflow: 'auto', flex: 1 }}>
        ...
        >
                    ...
                    <g key={`in-${idx}`} transform={`translate(0, ${y})`}>
                      ...
                      >
                        <rect 
                          width={60} 
                          height={rowHeight} 
                          fill={isSelected ? '#ffe066' : isConnected ? '#b2f2ff' : '#fff'}
                        />
                        <text
                          x={30}
                          y={rowHeight / 2}
          ...
          {fiberConnections.map((conn, i) => {
            ...
            const y1 = conn.from.fiberIdx * rowHeight + rowHeight / 2 + topPadding;
            const y2 = conn.to.fiberIdx * rowHeight + rowHeight / 2 + 
                      (outgoingRowOffsets[toCable.id] || 0) * rowHeight + 
                      Math.floor((outgoingCables.findIndex(c => c.id === parseInt(conn.to.cableId))) * cableSpacing) + 
                      topPadding;

            // Точные координаты соединительных точек
            const startX = 320; // Учитываем позицию точки на входящем кабеле (20 + 240 + 60)
            const endX = totalWidth - 380; // Учитываем позицию точки на исходящем кабеле

            // Используем S-образную кривую Безье для более плавного соединения
            ...
            return (
              <path
                key={i}
                d={`M ${startX} ${y1} C ${controlX1} ${y1}, ${controlX2} ${y2}, ${endX} ${y2}`}
                fill="none"
                stroke="#0070c0"
                strokeWidth={2}
              />

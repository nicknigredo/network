...
export default function SpliceClosureDetailDialog({
    spliceClosure,
    onClose,
    cables,
    spliceClosures,
    ...
    const [splitters, setSplitters] = useState<Splitter[]>(spliceClosure?.splitters || []);
    const [internalConnections, setInternalConnections] = useState<InternalConnection[]>(spliceClosure?.internalFiberConnections || []);
  
    useEffect(() => {
      // Обновляем состояния при изменении пропса spliceClosure
      setSplitters(spliceClosure?.splitters || []);
      setInternalConnections(spliceClosure?.internalFiberConnections || []);
    }, [spliceClosure]);
    
    const incomingCable = cables.find(c => c.targetElement?.type === 'spliceClosure' && c.targetElement.id === spliceClosure?.id);
    const outgoingCables = cables.filter(c => c.sourceElement.type === 'spliceClosure' && c.sourceElement.id === spliceClosure?.id);
    ...
    const renderCableInfo = (cable: Cable, side: 'left' | 'right', y: number) => {
      const fromBox = spliceClosures.find(b => b.id === cable.sourceElement.id);
      const toBox = spliceClosures.find(b => b.id === cable.targetElement?.id);
    ...
    const isConnectionPointBusy = (point: ConnectionPoint) => {
      // Проверяем ЗАНЯТОСТЬ только в рамках внутренних соединений текущей муфты
    ...
    const handleAddSplitter = (level: Splitter['level']) => {
      ...
      onUpdateSpliceClosureSplitters(spliceClosure.id, updatedSplitters);
    };
    ...
    const handleDeleteSplitter = (splitterId: string) => {
      ...
      onUpdateSpliceClosureSplitters(spliceClosure.id, updatedSplitters);
      onUpdateSpliceClosureInternalConnections(spliceClosure.id, updatedInternalConnections); // Обновляем состояние в App.tsx
    };
    ...
    const generateConnectionReport = () => {
        ...
        if (internalConnections.length === 0) {
            ...
            internalConnections.forEach((conn, i) => {
                const end1Str = formatConnectionPoint(conn.end1, spliceClosure.id);
                const end2Str = formatConnectionPoint(conn.end2, spliceClosure.id);
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          <h2 style={{ margin: 0 }}>Детали муфты №{spliceClosure.number}</h2>
        ...
        <div style={{ overflow: 'auto', flex: 1 }}>
          ...
          <div style={{ padding: '10px 20px 10px 20px', borderBottom: '1px dashed #eee', marginBottom: '10px' }}>
            ...
            <div style={{ marginBottom: '10px', display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
              ...
              <button 
                onClick={() => {
                  if (!spliceClosure) return;
                  // Простая проверка для муфт
                  const errors: string[] = [];
                  
                  if (splitters.length === 0) {
                    errors.push(`Муфта ${spliceClosure.number}: отсутствуют сплиттеры`);
                  }
                  
                  if (internalConnections.length === 0) {
                    errors.push(`Муфта ${spliceClosure.number}: отсутствуют внутренние соединения волокон`);
                  }
          ...
          >
            ...
            {internalConnections.map((conn, i) => { // Теперь используется локальное состояние internalConnections
              ...
              if (conn.end1.type === 'cableFiber') {
                ...
                if (cable.targetElement?.type === 'spliceClosure' && cable.targetElement.id === spliceClosure?.id) {
                ...
                else if (cable.sourceElement.type === 'spliceClosure' && cable.sourceElement.id === spliceClosure?.id) {
              ...
              return (
                <path
                  ...
                  onClick={() => {
                    ...
                    onUpdateSpliceClosureInternalConnections(spliceClosure.id, updatedInternalConnections);
                  }}
                >
        ...
        {showConnectionsReport && (
            <div style={{
                ...
                <div style={{
                    ...
                    <h3 style={{ margin: 0 }}>Отчет по соединениям муфты №{spliceClosure.number}</h3>

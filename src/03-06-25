...
import { MapContainer, TileLayer, Marker, ScaleControl, useMapEvents, Popup, Tooltip, Polyline, LayerGroup, LayersControl } from 'react-leaflet';
...
function App() {
  ...
  return (
    <div style={{ height: "100vh", width: "100vw", display: 'flex' }}> {/* Добавляем flex контейнер */}
      {/* Левая панель с деревом элементов */}
      <div style={{
        width: 280, // Ширина панели
        background: '#f8f8f8',
      ...
      <div style={{ position: 'relative', flexGrow: 1 }}> {/* Делаем контейнер карты гибким */}
        <Toolbar
      ...
      <div style={{
        ...
        <b>Таблица кабелей (для проверки):</b>
        <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: 6 }}>
          ...
          <tbody>
            {cables.map(cable => (
              <tr key={cable.id}>
                <td>{cable.id}</td>
                <td>{cable.fiberCount}</td>
                <td>{cable.layingType}</td>
                <td>{calculateCableLength(cable.points).toFixed(1)} м</td>
                <td>{cable.points.length}</td>
                <td style={{ fontSize: 11 }}>{cable.points.map(pt => pt.map(x => x.toFixed(5)).join(',')).join(' | ')}</td>
              </tr>
            ))}
          </tbody>
        </table>
